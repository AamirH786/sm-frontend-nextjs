...
import { deliveryService, getMasterAvatarAssignmentCount, isMasterAssignedToAvatar, Master } from '@/services/mastersService';
...
export default function DeliveryPage() {
  ...
  const handleToggle = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned delivery styles cannot be deactivated while linked to an avatar.', 'error');
      return;
    }

    try {
      await deliveryService.toggle(row.id);
  ...
  const handleDelete = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned delivery styles cannot be deleted while linked to an avatar.', 'error');
      return;
    }

    if (!confirm('Are you sure you want to delete this delivery style?')) return;
    try {
      await deliveryService.delete(row.id);
      showToast('Delivery style deleted', 'success');
      reloadTable();
    } catch (err: any) {
      showToast(err.message, 'error');
    }
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editing) {
        ...
        showToast('Delivery style updated successfully', 'success');
      } else {
        await deliveryService.create(data);
        showToast('Delivery style created successfully', 'success');
      }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={handleOpenCreate}>
            <Plus size={18} className="mr-2" /> Add Delivery
          </Button>
        </div>
      </div>
      ...
      <Modal
        ...
        title={editing ? 'Edit Delivery Style' : 'Add Delivery Style'}
      >
