...
import { getMasterAvatarAssignmentCount, isMasterAssignedToAvatar, Master, systemSafetyService } from '@/services/mastersService';
...
export default function SystemSafetyPage() {
  ...
  const handleToggle = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned system safety records cannot be deactivated while linked to an avatar.', 'error');
      return;
    }

    try {
      await systemSafetyService.toggle(row.id);
  ...
  const handleDelete = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned system safety records cannot be deleted while linked to an avatar.', 'error');
      return;
    }

    if (!confirm('Are you sure you want to delete this system safety record?')) return;
    try {
      await systemSafetyService.delete(row.id);
      showToast('System Safety deleted', 'success');
      reloadTable();
    } catch (err: any) {
      showToast(err.message, 'error');
    }
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editing) {
        await systemSafetyService.update(editing.id, data);
        showToast('System Safety updated successfully', 'success');
      } else {
        await systemSafetyService.create(data);
        showToast('System Safety created successfully', 'success');
      }
  ...
  const fetchTable = async ({ page, limit, search, sortBy, sortOrder }: any) => {
    ...
    const res = await systemSafetyService.list(params);
    return {
      data: res.data,
      pagination: {
        page: res.meta.page,
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-xl font-semibold">System Safety</h2>
        <div className="flex gap-3">
          ...
          <Button onClick={handleOpenCreate}>
            <Plus size={18} className="mr-2" /> Add System Safety
          </Button>
        </div>
      </div>
      ...
      <Modal
        ...
        title={editing ? 'Edit System Safety' : 'Add System Safety'}
      >
