...
import { domainsService, getMasterAvatarAssignmentCount, isMasterAssignedToAvatar, Master } from '@/services/mastersService';
...
export default function DomainsPage() {
  ...
  const handleToggle = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned domains cannot be deactivated while linked to an avatar.', 'error');
      return;
    }

    try {
      await domainsService.toggle(row.id);
  ...
  const handleDelete = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned domains cannot be deleted while linked to an avatar.', 'error');
      return;
    }

    if (!confirm('Are you sure you want to delete this domain?')) return;
    try {
      await domainsService.delete(row.id);
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editing) {
        await domainsService.update(editing.id, data);
  ...
  const fetchTable = async ({ page, limit, search, sortBy, sortOrder }: any) => {
    ...
    const res = await domainsService.list(params);
    return {
      data: res.data,
      pagination: {
        page: res.meta.page,
