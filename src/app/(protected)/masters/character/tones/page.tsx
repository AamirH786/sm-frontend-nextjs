...
import { getMasterAvatarAssignmentCount, isMasterAssignedToAvatar, Master, tonesService } from '@/services/mastersService';
...
export default function TonesPage() {
  ...
  const handleToggle = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned tones cannot be deactivated while linked to an avatar.', 'error');
      return;
    }

    try {
      await tonesService.toggle(row.id);
  ...
  const handleDelete = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned tones cannot be deleted while linked to an avatar.', 'error');
      return;
    }

    if (!confirm('Are you sure you want to delete this tone?')) return;
    try {
      await tonesService.delete(row.id);
      showToast('Tone deleted', 'success');
      reloadTable();
    } catch (err: any) {
      showToast(err.message, 'error');
    }
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editing) {
        await tonesService.update(editing.id, data);
        showToast('Tone updated successfully', 'success');
      } else {
        await tonesService.create(data);
        showToast('Tone created successfully', 'success');
      }
  ...
  const fetchTable = async ({ page, limit, search, sortBy, sortOrder }: any) => {
    ...
    const res = await tonesService.list(params);
    return {
      data: res.data,
      pagination: {
        page: res.meta.page,
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-xl font-semibold">Tones</h2>
        <div className="flex gap-3">
          ...
          <Button onClick={handleOpenCreate}>
            <Plus size={18} className="mr-2" /> Add Tone
          </Button>
        </div>
      </div>
      ...
      <Modal
        ...
        title={editing ? 'Edit Tone' : 'Add Tone'}
      >
