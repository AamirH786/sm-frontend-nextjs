...
import { getMasterAvatarAssignmentCount, isMasterAssignedToAvatar, Master, modesService } from '@/services/mastersService';
...
export default function ModesPage() {
  ...
  const handleToggle = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned modes cannot be deactivated while linked to an avatar.', 'error');
      return;
    }

    try {
      await modesService.toggle(row.id);
  ...
  const handleDelete = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned modes cannot be deleted while linked to an avatar.', 'error');
      return;
    }

    if (!confirm('Are you sure you want to delete this mode?')) return;
    try {
      await modesService.delete(row.id);
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editing) {
        await modesService.update(editing.id, data);
        showToast('Mode updated successfully', 'success');
      } else {
        await modesService.create(data);
        showToast('Mode created successfully', 'success');
      }
  ...
  const fetchTable = async ({ page, limit, search, sortBy, sortOrder }: any) => {
    ...
    const res = await modesService.list(params);
    return {
      data: res.data,
      pagination: {
        page: res.meta.page,
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={handleOpenCreate}>
            <Plus size={18} className="mr-2" /> Add Mode
          </Button>
        </div>
      </div>
      ...
      <Modal
        ...
        title={editing ? 'Edit Mode' : 'Add Mode'}
      >
