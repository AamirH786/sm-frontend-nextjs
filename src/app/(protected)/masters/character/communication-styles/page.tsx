...
import { communicationStylesService, getMasterAvatarAssignmentCount, isMasterAssignedToAvatar, Master } from '@/services/mastersService';
...
export default function CommunicationStylesPage() {
  ...
  const handleToggle = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned communication styles cannot be deactivated while linked to an avatar.', 'error');
      return;
    }

    try {
      await communicationStylesService.toggle(row.id);
  ...
  const handleDelete = async (row: Master) => {
    if (isMasterAssignedToAvatar(row as Master & Record<string, unknown>)) {
      showToast('Assigned communication styles cannot be deleted while linked to an avatar.', 'error');
      return;
    }

    if (!confirm('Are you sure you want to delete this communication style?')) return;
    try {
      await communicationStylesService.delete(row.id);
      showToast('Communication Style deleted', 'success');
      reloadTable();
    } catch (err: any) {
      showToast(err.message, 'error');
    }
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editing) {
        await communicationStylesService.update(editing.id, data);
        showToast('Communication Style updated successfully', 'success');
      } else {
        await communicationStylesService.create(data);
        showToast('Communication Style created successfully', 'success');
      }
  ...
  const fetchTable = async ({ page, limit, search, sortBy, sortOrder }: any) => {
    ...
    const res = await communicationStylesService.list(params);
    return {
      data: res.data,
      pagination: {
        page: res.meta.page,
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-xl font-semibold">Communication Styles</h2>
        <div className="flex gap-3">
          ...
          <Button onClick={handleOpenCreate}>
            <Plus size={18} className="mr-2" /> Add Style
          </Button>
        </div>
      </div>
      ...
      <Modal
        ...
        title={editing ? 'Edit Communication Style' : 'Add Communication Style'}
      >
