...
import { interestsService, OnboardingMaster } from '@/services/onboardingService';
...
export default function InterestsPage() {
  ...
  const fetchData = useCallback(async (params: { page: number; limit: number; search: string }) => {
    try {
      ...
      showToast(err.message || 'Failed to load interests', 'error');
      return {
        data: [],
  ...
  const handleToggleStatus = async (item: OnboardingMaster) => {
    try {
      await interestsService.toggleStatus(item.id);
  ...
  const onSubmitAdd = async (data: FormValues) => {
    try {
      setSaving(true);
      await interestsService.create(normalizePayload(data));
      showToast('Interest created successfully', 'success');
      setShowAddModal(false);
      reset();
      setRefreshTrigger(prev => prev + 1);
    } catch (err: any) {
      showToast(err.message || 'Failed to create interest', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const onSubmitEdit = async (data: FormValues) => {
    ...
    try {
      setSaving(true);
      await interestsService.update(editItem.id, normalizePayload(data));
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await interestsService.delete(deleteItem.id);
      showToast('Interest deleted successfully', 'success');
      setDeleteItem(null);
      setRefreshTrigger(prev => prev + 1);
    } catch (err: any) {
      showToast(err.message || 'Failed to delete interest', 'error');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Manage user interests for onboarding</p>
          </div>
        </div>
      ...
      <Modal isOpen={showAddModal} onClose={() => { setShowAddModal(false); setIsKeyManuallyEdited(false); }} title="Add Interest">
      ...
      <Modal isOpen={!!editItem} onClose={() => { setEditItem(null); setIsKeyManuallyEdited(false); }} title="Edit Interest">
      ...
      <Modal isOpen={!!viewItem} onClose={() => setViewItem(null)} title="View Interest">
        {viewItem && (
      ...
      <Modal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} title="Delete Interest">
        <div className="space-y-4">
